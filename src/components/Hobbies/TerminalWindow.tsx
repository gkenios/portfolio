...
export function TerminalWindow({ codeSnippet, accentColor }: TerminalWindowProps) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setCharIndex((i) => (i >= codeSnippet.length ? 0 : i + 1)), delay);
  ...
  return (
    <div
      className="w-full h-full p-3 flex flex-col rounded-xl overflow-hidden"
